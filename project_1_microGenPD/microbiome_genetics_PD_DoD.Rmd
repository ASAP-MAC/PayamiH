...
### The other way around: model dosage in each Case status and record where they are discordant
...
with genetic variants in interaction with PD. We can use `edgeR` for counts, as 
suggested by Levi after a recent benchmark as showing a nice overlap with 
expected signals based on well characterized microbiome datasets. For relative
abundance data, maybe `maaslin3` or `linda` is a good option (NB: Maaslin2 is 
not maintained in BioC version 3.20). After testing,we can perform a gene set 
enrichment analysis using as gene sets:
...
Second one is broader, so it could be an extention if we think it is necessary.
