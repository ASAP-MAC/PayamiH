---
title: "PD, microbiome and host genetics on SNCA and HLA variants on DoD"
...
Below is the data for the DoD dataset. Data have been 
...
```{r}
prevalence_curve <- assay(DoD_data.mae, "metaphlan_Counts.se") %>% 
...
genotypes_vs_microbiome_linda.list <- sapply(grep("^chr", colnames(data_alltogether_Q1.df), value = TRUE), 
       ...
       suppressMessages(linda(
         feature.dat = assay(DoD_data.mae, "metaphlan_Counts.se")[, data_alltogether_Q1.df$SubjectID],
...
genotypes_vs_microbiome_linda.list <- sapply(grep("^chr", colnames(data_alltogether_Q2.df), value = TRUE), 
       function(gt)
       suppressMessages(linda(
         feature.dat = assay(DoD_data.mae, "metaphlan_Counts.se")[,data_alltogether_Q2.df$SubjectID],
...
genotypes_vs_microbiome_linda.list <- sapply(grep("^chr", colnames(data_alltogether_Q3.df), value = TRUE), 
       function(gt)
       suppressMessages(linda(
         feature.dat = assay(DoD_data.mae, "metaphlan_Counts.se")[,data_alltogether_Q3.df$SubjectID],
...
# Results highlights
  ...
  - In the interaction model using clusterSums, the DoD dataset show 2 interesting
  trends on 2 variants
  - In the vast majority of cases, associations never reach formal statistical 
  significance
  - With nominal p-value < 0.1 some cluster-variant associations show betas in 
  opposite directions in PD-only (Q2) and controls-only (Q3): Streptos-Actinos 
  in PD go in opposite directions in association with 2 SNCA variants: 89587189 
  (up), 89715479 (down)

Gene set enrichment highlighted more associations than linear models did: 

Variant chr6_32698883 shows enrichment of Fiber degraders controls but not in PD,
as well as chr4_89840793 (aka **our top variant**).

Overall, we are seeing a complex interplay, that is more easily seen in gene 
set enrichment analyses. More generally, trends visible in controls are not visible
in PD and vice versa. 

These may be trends, however, with larger sample sizes, more interactions would 
be visible.
...
snp_data <- data.frame(
  ...
  mutate(
    ...
    AlreadyInList = ifelse((rsID %in% rownames(rowData(DoD_data.mae[["genotypes_dosages.se"]]))) | (AltName %in% rownames(rowData(DoD_data.mae[["genotypes_dosages.se"]]))), "yes", "no")
  ) %>% 
  arrange(AlreadyInList)

kbl(snp_data, caption = "List of variants found in FoxDen data repository. Grayed rows indicate variants already in our selection criteria in the GWAS catalog.") %>% 
  kable_styling() %>% 
...
dataTable <- readxl::read_xlsx("../Data/GWAS_catalog_PD_variants/Nalls_2023_Table1Variants.xlsx") %>% 
  mutate(AlreadyInList = ifelse(rsID %in% rownames(rowData(DoD_data.mae[["genotypes_dosages.se"]])), "yes", "no")) %>% 
  arrange(AlreadyInList) 

kbl(dataTable, caption = "List of variants associated with PD reported as 'novel' in a mega GWAS by the authors in Nalls E.A. et al, 2024 (https://doi.org/10.1016/S1474-4422(19)30320-5). Grayed rows indicate variants already in our selection criteria in the GWAS catalog.") %>% 
  kable_styling() %>% 
