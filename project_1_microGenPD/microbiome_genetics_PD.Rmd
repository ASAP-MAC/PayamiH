...
load("../Data/WallenZD_2022_data.mae.RData")
...
cluster_sums.df <- left_join(biomarkers_medrxiv.df, 
          as.data.frame(assay(WallenZD_2022_data.mae, "metaphlan_RelAbund.se")/100) %>%
...
samples_to_keep.df <- colData(WallenZD_2022_data.mae[["metaphlan_RelAbund.se"]]) %>%
  as.data.frame() %>% 
  filter(
...
variants_to_keep_rowData.df <- as.data.frame(rowData(WallenZD_2022_data.mae[["genotypes_dosages.se"]])[grepl("HLA|SNCA", rowData(WallenZD_2022_data.mae[["genotypes_dosages.se"]])$GENE),])
rownames(variants_to_keep_rowData.df) <- variants_to_keep_rowData.df$RSID

per_sample_dosages.df <- t(assay(WallenZD_2022_data.mae, "genotypes_dosages.se"))[,rownames(variants_to_keep_rowData.df)] %>% 
  as.data.frame() %>% 
...
genotypes_vs_microbiome_linda.list <- sapply(grep("^chr", colnames(meta_with_variants), value = TRUE), 
  ...
  otu.tab = assay(WallenZD_2022_data.mae, "metaphlan_Counts.se")[,meta_with_variants$sample_name],
...
```{r DirichletMultinomial calculation, cache=TRUE}
library(DirichletMultinomial)

counts_tab <- t(assay(WallenZD_2022_data.mae, "metaphlan_Counts.se"))
...
dir.create("../Data/wallen2022_DirichletMultinomial/", showWarnings = FALSE)
saveRDS(dmn_raw_output, file = paste0("../Data/wallen2022_DirichletMultinomial/", "DMN_raw_output_", Sys.Date(), ".Rds"))
```
...
laplace_values <- sapply(dmn_raw_output$dmn_output, laplace)
plot(laplace_values, type = "b", col = "darkgray", xlab = "N. Dirichlet Components modeled", ylab = "Laplace Fit evaluation")
...
clusters_arbitraryN <- dmn_raw_output$dmn_output[[k]]
