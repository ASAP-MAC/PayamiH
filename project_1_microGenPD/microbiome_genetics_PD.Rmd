...
signif_assocs <- lm_split_by_variant_and_dosage %>%
  filter(term == "Case_statusPD", p.adj < 0.1)
...
summary_table <- lm_split_by_variant_and_dosage %>% 
  ...
  .[,c(1,2,3,6,4,7,5,8)] %>% 
  ungroup() %>% 
  filter(as.numeric(p_adj_0) < 0.1 | as.numeric(p_adj_1) < 0.1 | as.numeric(p_adj_2) < 0.1) %>% 
  mutate(
...
lm_split_by_variant_and_dosage %>% 
  ...
  geom_text(
    ...
    mapping = aes(x = variant, y = yposit, label = ifelse(p.adj < 0.1, "*", ""), group = dosage), 
