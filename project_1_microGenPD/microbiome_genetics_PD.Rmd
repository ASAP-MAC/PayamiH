...
variants_to_test_LD_matrices_molten <- split.data.frame(variants_to_keep_rowData.df,variants_to_keep_rowData.df$CHROM) %>% 
  lapply( 
    function(x)
    LDlinkR::LDmatrix(
      snps = x$RSID,
...
variants_to_test_LD_matrices_molten_notHighLD <- lapply(variants_to_test_LD_matrices_molten, mutate, 
                                                        Var2 = case_when(value >= 0.8 ~ NA, 
                                                                         TRUE ~ Var2))
...
n_indep_variants <- sum(sapply(variants_pruning_step, function(x) sum(grepl("kept", x$Details))))
```
...
## Odds ratio PD/healthy split by genotype dosage (0, 1, 2)

For each genotype in variant `k`, model as following

$$
ClusterSum_i \sim Age + Seq.Depth + PD
$$

And extract the Fold Change of each cluster for PD $FC_i = \frac{PD}{Ctrl}$

```{r}
lm_grouped_by <- data_alltogether.df %>%
  pivot_longer(
  ...
  filter(sum(Case_status == "PD") >= 3 & sum(Case_status == "Control") >= 3) %>%  # Keep only groups with at least 2 observations
  group_modify(~ {
  ...
  # adjust p-values by factor
  mutate(
    p.adj = ifelse(p.value * n_tests >= 1, 1, p.value * n_tests)
  )
```

```{r table split results}
summary_table <- lm_grouped_by %>%
  filter(term == "Case_statusPD", p.value < 0.05) %>%
  ...
  ungroup() %>% 
  mutate(
    p_adj_0 = ifelse(as.numeric(p_adj_0) < 0.1, paste(p_adj_0, "*"), p_adj_0),
    p_adj_1 = ifelse(as.numeric(p_adj_1) < 0.1, paste(p_adj_1, "*"), p_adj_1),
    p_adj_2 = ifelse(as.numeric(p_adj_2) < 0.1, paste(p_adj_2, "*"), p_adj_2)
  )

highlight_rows <- which(
  as.numeric(gsub(" *", "", summary_table$p_adj_0, fixed = TRUE)) < 0.1 | 
    as.numeric(gsub(" *", "", summary_table$p_adj_1, fixed = TRUE)) < 0.1 |
    as.numeric(gsub(" *", "", summary_table$p_adj_2, fixed = TRUE)) < 0.1
  )


summary_table %>% 
  as.matrix() %>% 
  magrittr::set_colnames(c("clusterName", "variant", rep(c("Beta", "P.adj"), 3))) %>%
  kable("html", caption = "Overview of clusters' associations with PD separated by each variant's ID. Only variants with at least 1 association were kept in the visualization. PD-cluster associations that passed Bonferroni test as shown above, were marked with an asterisk ('*') on the p.adj column.") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = T) %>%
  add_header_above(c(" " = 2, "Dosage 0" = 2, "Dosage 1" = 2, "Dosage 2" = 2)) %>%
  column_spec(1:2, bold = T) %>%
  collapse_rows(columns = 1, valign = "top") %>% 
  row_spec(highlight_rows, bold = TRUE, color = "black")
```

```{r}
plot_data <- lm_grouped_by %>%
  filter(term == "Case_statusPD", p.value < 0.05) %>%
  mutate(variant = factor(variant, levels = sort(unique(variant))))

ggplot(plot_data, aes(x = variant, y = estimate, fill = dosage)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.8), width = 0.7) +
  # geom_errorbar(aes(ymin = estimate - std.error, ymax = estimate + std.error),
  #               position = position_dodge(width = 0.8), width = 0.25) +
  facet_grid(cols = vars(clusterName), rows = vars(variant), scales = "free_y") +
  coord_flip() +
  labs(x = "Variant", y = "Estimate", fill = "Dosage") +
  theme(
    axis.text.y = element_blank(),
    strip.text.y = element_text(angle = 0, vjust = 0.5, hjust = 0, color = "black"),
    strip.background.y = element_rect(fill = "white", size = 8),
