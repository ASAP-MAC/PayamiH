...
cd /data/project/payami-lab/payamilab/TOPMed_imputed_genotypes_2022/
...
for chr in $(ls | grep -oE "^(chr[0-9]*|chrX)"); do bcftools query -H -f "%CHROM\t%POS\t%REF\t%ALT\t%INFO[\t%DS]\n" -r $(tr "\n" "," < ${chr}_variants_Tabix.txt | sed "s/,$//") /data/project/payami-lab/payamilab/TOPMed_imputed_genotypes_2022/${chr}.dose.vcf.gz | sed -e "s/#//g" -e "s/:DS//g" -e "s/\[[0-9]*\]//g" -e "s/(null)/INFO/g" > queried_variants_PD/${chr}_pd_Dosages.txt; done

for chr in $(ls | grep -oE "^(chr[0-9]*|chrX)"); do bcftools query -H -f "%CHROM\t%POS\t%REF\t%ALT\t%INFO[\t%GT]\n" -r $(tr "\n" "," < ${chr}_variants_Tabix.txt | sed "s/,$//") /data/project/payami-lab/payamilab/TOPMed_imputed_genotypes_2022/${chr}.dose.vcf.gz | sed -e "s/#//g" -e "s/:GT//g" -e "s/\[[0-9]*\]//g" -e "s/(null)/INFO/g" > queried_variants_PD/${chr}_pd_Genotypes.txt; done

```

```{bash eval=FALSE}
rm /data/project/payami-lab/payamilab/TOPMed_imputed_genotypes_2022/slurm*
```
