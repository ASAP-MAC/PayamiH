...
```{r}
# get sample metadata
UAB_metadata <- as.data.frame(readxl::read_xlsx("/data/project/payami-lab/payamilab/datasets_sample_renaming/UAB_new_SourceFile.xlsx", sheet = "subject_metadata"))
rownames(UAB_metadata) <- UAB_metadata$SubjectID

# get Metaphlan relative abundances
UAB_metaphlan_relab <- readxl::read_xlsx("/data/project/payami-lab/payamilab/datasets_sample_renaming/UAB_new_SourceFile.xlsx", sheet = "metaphlan_rel_ab") %>% 
  wrangle_metaphlan("Species")

# get Metaphlan estimated counts
UAB_metaphlan_counts <- readxl::read_xlsx("/data/project/payami-lab/payamilab/datasets_sample_renaming/UAB_new_SourceFile.xlsx", sheet = "metaphlan_counts") %>% wrangle_metaphlan("Species")
UAB_metaphlan_counts$profiles <- round(UAB_metaphlan_counts$profiles, 0)

# get HUMAnN KO tables
UAB_humann_KO <- readxl::read_xlsx("/data/project/payami-lab/payamilab/datasets_sample_renaming/UAB_new_SourceFile.xlsx", sheet = "humann_KO_group_counts") %>% 
  wrangle_humann(input_type = "ko")
UAB_humann_KO$profiles <- round(UAB_humann_KO$profiles, 0)

# get HUMAnN Pathway tables
UAB_humann_pathways <- readxl::read_xlsx("/data/project/payami-lab/payamilab/datasets_sample_renaming/UAB_new_SourceFile.xlsx", sheet = "humann_pathway_counts") %>% wrangle_humann(input_type = "pathways")
...
samples_to_exclude.df %>% 
  ...
  kbl(caption = "List of samples excluded from the UAB dataset, along with reason for exclusion.")
```
