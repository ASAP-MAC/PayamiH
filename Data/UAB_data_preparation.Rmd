---
title: "UAB data preparation into MultiAssayExperiment format"
...
```{r}
# get sample metadata
UAB_metadata <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_NGRC_UAB515.xlsx", sheet = "subject_metadata") %>% 
  as.data.frame() %>% 
  # this is to get only UAB samples
  filter(Location == "Birmingham_AL")
rownames(UAB_metadata) <- UAB_metadata$SubjectID

# get Metaphlan relative abundances
UAB_metaphlan_relab <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_NGRC_UAB515.xlsx", sheet = "metaphlan_rel_ab") %>% 
  wrangle_metaphlan("Species")
UAB_metaphlan_relab$profiles <- UAB_metaphlan_relab$profiles[,UAB_metadata$sample_name] %>% magrittr::set_colnames(rownames(UAB_metadata))

# get Metaphlan estimated counts
UAB_metaphlan_counts <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_NGRC_UAB515.xlsx", sheet = "metaphlan_counts") %>% wrangle_metaphlan("Species")
UAB_metaphlan_counts$profiles <- round(UAB_metaphlan_counts$profiles, 0)
UAB_metaphlan_counts$profiles <- UAB_metaphlan_counts$profiles[,UAB_metadata$sample_name] %>% magrittr::set_colnames(rownames(UAB_metadata))

# get HUMAnN KO tables
UAB_humann_KO <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_NGRC_UAB515.xlsx", sheet = "humann_KO_group_counts") %>% 
  wrangle_humann(input_type = "ko")
UAB_humann_KO$profiles <- round(UAB_humann_KO$profiles, 0)
UAB_humann_KO$profiles <- UAB_humann_KO$profiles[, UAB_metadata$sample_name] %>% magrittr::set_colnames(rownames(UAB_metadata))

# get HUMAnN Pathway tables
UAB_humann_pathways <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_NGRC_UAB515.xlsx", sheet = "humann_pathway_counts") %>% wrangle_humann(input_type = "pathways")
UAB_humann_pathways$profiles <- round(UAB_humann_pathways$profiles, 0)
UAB_humann_pathways$profiles <- UAB_humann_pathways$profiles[, UAB_metadata$sample_name] %>% magrittr::set_colnames(rownames(UAB_metadata))

# find samples intersection across experiments and metadata

samples_per_data_piece <- list(
  "Metaphlan (Relative Abundance)" = colnames(UAB_metaphlan_relab$profiles),
  "Metaphlan (Counts)" = colnames(UAB_metaphlan_counts$profiles),
  "HUMAnN3 (Kegg Orthology)" = colnames(UAB_humann_KO$profiles),
  "HUMAnN3 (MetaCyc Pathways)" = colnames(UAB_humann_pathways$profiles),
  "Metadata" = rownames(UAB_metadata)
  )

samples_intersect <- reduce(samples_per_data_piece, intersect)

# load samples to exclude (not necessary in DoD, but necessary in UAB and UAB)
samples_to_exclude <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/exclusions NGRC UAB_ZW_HP.xlsx", sheet = "R-friendly list") %>% 
  mutate(sample_name = make.names(sample_name))

samples_intersect_after_exclusion <- samples_intersect[!(samples_intersect %in% samples_to_exclude$sample_name)]

# reorder colnames so they all match the metadata

UAB_metaphlan_relab$profiles <- UAB_metaphlan_relab$profiles[, samples_intersect_after_exclusion]
UAB_metaphlan_counts$profiles <- UAB_metaphlan_counts$profiles[, samples_intersect_after_exclusion]
UAB_humann_KO$profiles <- UAB_humann_KO$profiles[, samples_intersect_after_exclusion]
UAB_humann_pathways$profiles <- UAB_humann_pathways$profiles[, samples_intersect_after_exclusion]

sample_metadata.df <- UAB_metadata[samples_intersect_after_exclusion,] %>% 
...
## Continue from here - Find which metagenomics samples also have genetic data 

For UAB, many samples are named differently. For this reason, we need one extra 
layer of mapping:

```{r}
mapping_names_UAB <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/NGRC_Hamza_GWAS_microbiome_genotype_ID_map.xlsx") %>% 
  set_names(c("microbiome_id", "genotype_id"))

as_tibble(mapping_names_UAB)
...
names(variant_metadata_cols) <- variant_metadata_cols

sampleIDs_remapped <- sapply(
  rownames(sample_metadata.df), 
  function(x) 
    ifelse(purrr::is_empty(mapping_names_UAB$genotype_id[grep(x, mapping_names_UAB$microbiome_id)]),
           x, mapping_names_UAB$genotype_id[grep(x, mapping_names_UAB$microbiome_id)]), 
  simplify = FALSE, 
  USE.NAMES = TRUE)

sampleIDs_TOPMed_remapped <- lapply(sampleIDs_remapped, function(x) grep(x, colnames(gwasCatalog_variants_dosages_in_TOPMed.df), value = TRUE)) %>% 
  unlist()

newColnames <- c(variant_metadata_cols, sampleIDs_TOPMed_remapped)

gwasCatalog_variants_dosages_in_TOPMed_selectSamps.df <- select(gwasCatalog_variants_dosages_in_TOPMed.df, all_of(newColnames))
gwasCatalog_variants_hardcalls_in_TOPMed_selectSamps.df <- select(gwasCatalog_variants_hardcalls_in_TOPMed.df, all_of(newColnames))
...
tmp <- gwasCatalog_variants_dosages_in_TOPMed_selectSamps.df
...
tmp <- gwasCatalog_variants_hardcalls_in_TOPMed_selectSamps.df
rownames(tmp) <- variants_made_unique
...
summarizedExperiments.list <- list(
  ...
  metaphlan_RelAbund.se = SummarizedExperiment(
    assays = list("metaphlan_RelAbund" = UAB_metaphlan_relab$profiles),
    rowData =  UAB_metaphlan_relab$taxonomies,
    colData = sample_metadata.df
  ),
  
  metaphlan_Counts.se = SummarizedExperiment(
    assays = list("metaphlan_Counts" = UAB_metaphlan_counts$profiles),
    rowData =  UAB_metaphlan_counts$taxonomies,
    colData = sample_metadata.df
  ),
  
  humann_KO.se = SummarizedExperiment(
    assays = list("humann_KO" = UAB_humann_KO$profiles),
    rowData =  UAB_humann_KO$description,
    colData = sample_metadata.df
  ),
  
  humann_pathways.se = SummarizedExperiment(
    assays = list("humann_pathways" = UAB_humann_pathways$profiles),
    rowData =  UAB_humann_pathways$description,
    colData = sample_metadata.df
  )
)
...
```{r}
colnames(UAB_data.mae[["metaphlan_RelAbund.se"]])[!(colnames(UAB_data.mae[["metaphlan_RelAbund.se"]]) %in% colnames(UAB_data.mae[["genotypes_dosages.se"]]))]
```
