...
DoD_metadata <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_24Oct2022 (DoD from Zenodo).xlsx", sheet = "subject_metadata") %>% 
  as.data.frame()
rownames(DoD_metadata) <- DoD_metadata$sample_name
...
#' This next step is crucial, because in many cases samples don't immediately match between 
#' microbiome and TOPMed data sets.
#' My strategy was to extract sample names with str_extract, because it returns 
#' only one match, the first.

names(newColnames) <- c(variant_metadata_cols, str_extract(sampleIDs, paste(rownames(sample_metadata.df), collapse = "|")))

# subset gwas datasets with available samples in the microbiome dataset
