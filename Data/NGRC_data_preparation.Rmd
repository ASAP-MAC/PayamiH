...
```{r}
# get sample metadata
NGRC_metadata <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_NGRC_UAB515.xlsx", sheet = "subject_metadata") %>% 
  as.data.frame() %>% 
  # this is to get only NGRC samples
  filter(Location != "Birmingham_AL")
rownames(NGRC_metadata) <- NGRC_metadata$SubjectID

# get Metaphlan relative abundances
NGRC_metaphlan_relab <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_NGRC_UAB515.xlsx", sheet = "metaphlan_rel_ab") %>% 
  wrangle_metaphlan("Species")
NGRC_metaphlan_relab$profiles <- NGRC_metaphlan_relab$profiles[,NGRC_metadata$sample_name] %>% magrittr::set_colnames(rownames(NGRC_metadata))

# get Metaphlan estimated counts
NGRC_metaphlan_counts <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_NGRC_UAB515.xlsx", sheet = "metaphlan_counts") %>% wrangle_metaphlan("Species")
NGRC_metaphlan_counts$profiles <- round(NGRC_metaphlan_counts$profiles, 0)
NGRC_metaphlan_counts$profiles <- NGRC_metaphlan_counts$profiles[,NGRC_metadata$sample_name] %>% magrittr::set_colnames(rownames(NGRC_metadata))

# get HUMAnN KO tables
NGRC_humann_KO <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_NGRC_UAB515.xlsx", sheet = "humann_KO_group_counts") %>% 
  wrangle_humann(input_type = "ko")
NGRC_humann_KO$profiles <- round(NGRC_humann_KO$profiles, 0)
NGRC_humann_KO$profiles <- NGRC_humann_KO$profiles[, NGRC_metadata$sample_name] %>% magrittr::set_colnames(rownames(NGRC_metadata))

# get HUMAnN Pathway tables
NGRC_humann_pathways <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/Source_Data_NGRC_UAB515.xlsx", sheet = "humann_pathway_counts") %>% wrangle_humann(input_type = "pathways")
NGRC_humann_pathways$profiles <- round(NGRC_humann_pathways$profiles, 0)
NGRC_humann_pathways$profiles <- NGRC_humann_pathways$profiles[, NGRC_metadata$sample_name] %>% magrittr::set_colnames(rownames(NGRC_metadata))
...
For NGRC, many samples are named differently. For this reason, we need one extra 
layer of mapping:

```{r}
mapping_names_NGRC <- readxl::read_xlsx("/data/project/payami-lab/payamilab/0_Source_Data/NGRC_Hamza_GWAS_microbiome_genotype_ID_map.xlsx") %>% 
  set_names(c("microbiome_id", "genotype_id"))

as_tibble(mapping_names_NGRC)
...
sampleIDs_remapped <- sapply(
  ...
  function(x) 
    ifelse(purrr::is_empty(mapping_names_NGRC$genotype_id[grep(x, mapping_names_NGRC$microbiome_id)]),
           x, mapping_names_NGRC$genotype_id[grep(x, mapping_names_NGRC$microbiome_id)]), 
  simplify = FALSE, 
  USE.NAMES = TRUE)
