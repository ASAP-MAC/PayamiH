...
```{r}
gwasCatalog_variants_All <- data.table::fread("GWAS_catalog_PD_variants/PD_variants_GWAS_catalog.tsv", data.table = FALSE) %>% 
...
gwasCatalog_variants_in_WallenZD.df <- list.files(path = "GWAS_catalog_PD_variants/queried_variants_PD/", pattern = "*Dosages.txt", full.names = T) %>% 
...
wallenZD_summarizedExperiments.list <- list(
  genotypes.se = SummarizedExperiment(
    assays = list("genotypes_dosages.se" = genetics_matrix),
