...
for chr in `ls | grep -oE "^(chr[0-9]*|chrX)"`; do bcftools query -s `cat ../sampleNames/sampleNamesPD_oneLine.txt` -H -f '%CHROM\t%POS\t%REF\t%ALT\t%INFO/R2[\t%GT]\n' -r `cat ${chr}_variants_Tabix.txt|tr "\n" "," | sed "s/\,$//g"`  ../TOPMed_imputed_genotypes/${chr}.dose.vcf.gz | sed -e "s/\#//g"| sed "s/\:GT//g" | sed -e "s/\[[0-9]*\]//g" > queried_variants_PD/${chr}_pd_Genotypes.txt; done

for chr in `ls | grep -oE "^(chr[0-9]*|chrX)"`; do bcftools query -s `cat ../sampleNames/sampleNamesPD_oneLine.txt` -H -f '%CHROM\t%POS\t%REF\t%ALT\t%INFO/R2[\t%DS]\n' -r `cat ${chr}_variants_Tabix.txt|tr "\n" "," | sed "s/\,$//g"`  ../TOPMed_imputed_genotypes/${chr}.dose.vcf.gz | sed -e "s/\#//g"| sed "s/\:DS//g" | sed -e "s/\[[0-9]*\]//g" > queried_variants_PD/${chr}_pd_Dosages.txt; done
```
